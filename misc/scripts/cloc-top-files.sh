...
desired_files=$(git ls-files | rev | cut -d'.' -f1 | rev | sort | uniq -c | sort -rg | egrep -v "($undesired_file_types)" | head -n $desired_num_files | awk '{print $2}' | sed -e 's/$/\$/' | sed -e 's/^/\./' | sed ':a; N; $!ba; s/\n/|/g')
