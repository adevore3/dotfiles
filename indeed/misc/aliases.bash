...
alias gci='f(){ git clone git@code.corp.indeed.com:$(get_repo_name)/$1.git; cd $1; unset -f f; }; f'
alias gciacp='f(){ git clone git@code.corp.indeed.com:$(get_repo_name)/$1.git; cd $1; ant clippy-project; unset -f f; }; f'
...
alias ogf='f(){ DISPLAY=:0 firefox -new-tab -url $GITLAB_BASE_URL/$(get_repo_product_group_and_name)/blob/master/$(find . -name *.java -type f | grep "$1" | grep -iv test | cut -c 3-); unset -f f; }; f'
alias ogft='f(){ DISPLAY=:0 firefox -new-tab -url $GITLAB_BASE_URL/$(get_repo_product_group_and_name)/blob/master/$(find . -name *.java -type f | grep "$1" | grep -i test | cut -c 3-); unset -f f; }; f'
