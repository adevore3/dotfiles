...
alias gcod='git checkout $(git_default_branch)'
...
alias gdfd='git_diff_from_default'
...
alias gmom='git merge --no-ff origin/$(git_default_branch)'
alias gp='git push'
...
alias grod='git pull --rebase && git rebase origin/$(git_default_branch)'
...
alias gtl='git for-each-ref --sort=creatordate refs/tags --format "%(creatordate:iso) %(refname:short)"'
alias gtlt='gtl | tail'
alias gtlg='gtl | grep'
alias gtlgt='f(){ gtlg $1 | tail | hl $1; unset -f f; }; f'
...
alias git_all_file_count='git ls-tree --name-only -r origin/$(git_default_branch) | wc -l'
alias git_base_url='f(){ local url=https://$(cat .git/config | grep "url = git" | cut -d@ -f2 | sed "s/:/\//" | head -n1); echo ${url%.git}; unset -f f; }; f'
alias git_branch_commit_count='git rev-list --count HEAD ^$(git_default_branch)'
alias git_checkout_all_deleted_files="git status | grep deleted | awk '{print \$2}' | xargs git checkout --"
alias git_diff_from_default='git diff origin/$(git_default_branch)..HEAD'
...
alias git_remove_untracked_branches=$'git checkout -q $(git_default_branch) && git fetch -q --prune && git branch -vv | awk \'/: gone]/{print $1}\' | xargs -r git branch -d'
alias git_remove_untracked_files=$'f(){ git status | grep "${1:-src}" | awk \'{print $1}\' | xargs rm -rf; unset -f f; }; f'
alias git_remove_untracked_unmerged_branches=$'git checkout -q $(git_default_branch) && git fetch -q --prune && git branch -vv | awk \'/: gone]/{print $1}\' | xargs -r git branch -D'
alias git_restore_all_staged_files=$'git status | egrep "new file|modified" | rev | awk \'{print $1}\' | rev | xargs git restore --staged'
alias git_restore_all_staged_new_files=$'git status | grep "new file" | awk \'{print $3}\' | xargs git restore --staged'
alias git_restore_all_staged_modified_files=$'git status | grep "modified" | awk \'{print $2}\' | xargs git restore --staged'
alias git_restore_all_modified_files=$'git status | grep "modified" | awk \'{print $2}\' | xargs git restore'
